...
def match_paths_orientations(pts, reference_points, is_closed):
    ...
    if len(pts) > 2 and len(reference_points) > 2:
        v1 = normalize_vector(subtract_vectors(pts[0]['point'], pts[2]['point']))
        v2 = normalize_vector(subtract_vectors(reference_points[0]['point'], reference_points[2]['point']))
    else:
        v1 = normalize_vector(subtract_vectors(pts[0]['point'], pts[1]['point']))
        v2 = normalize_vector(subtract_vectors(reference_points[0]['point'], reference_points[1]['point']))
